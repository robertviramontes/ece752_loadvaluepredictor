...
void ConstantVerificationUnit::processStoreAddress(ThreadID tid, 
	...
	bool found = false;
	auto itr = _cvuCAM[tid].begin();
	while (itr != _cvuCAM[tid].end()) {
		struct CAMEntry temp = *itr;
		if (temp.load_address == address) {
			DPRINTF(CVU, "[TID %d]: Found store address: 0x%x in CVU CAM\n", tid, address);
			itr = _cvuCAM[tid].erase(itr);
			++_numStoreHits;
			found = true;
			if (itr == _cvuCAM[tid].end()) break;
		}
		itr++;
	}
	if (!found) {
		DPRINTF(CVU, "[TID %d]: Address 0x%x not found in CVU CAM", tid, address);
		++_numStoreMiss;
	}
}
...
bool ConstantVerificationUnit::processLoadAddress(Addr pc, 
	...
	while (itr != _cvuCAM[tid].end()) {
		...
		if (temp.pc == pc && temp.lvpt_index == lvptIndex) {
			DPRINTF(CVU, "[TID %d] Load instruction: 0x%x matched in CVU CAM", tid, pc);
			++_numConstantHits;
	...
	++_numConstantMiss;
	DPRINTF(CVU, "[TID %d] Load instruction: 0x%x not found in CVU CAM", tid, pc);
	return false;
}

bool ConstantVerificationUnit::updateConstLoad(Addr pc, Addr address,
											   Addr lvptIndex, ThreadID tid) {
	DPRINTF(CVU, "[TID %d]: Adding load address: 0x%x with PC: 0x%x to CVU CAM", tid, address, pc);
	...
	if(_cvuCAM[tid].size() >= _numEntries) {
		// CAM is full, replace a block according to a set replacement policy
		DPRINTF(CVU, "[TID %d]: No space for load address: 0x%x in CVU CAM", tid, address);
		this->replaceBlock(temp, tid);
		return true;
	}
	_cvuCAM[tid].push_back(temp);
	return true;
}
...
void ConstantVerificationUnit::replaceBlock(struct CAMEntry new_entry, ThreadID tid) {
	...
	if(_replacementPolicy == RP_FIFO) {
		...
		DPRINTF(CVU, "Entry with PC: 0x%x being ejected from CVU CAM", itr_replace->pc);
		_cvuCAM[tid].erase(itr_replace);
		_cvuCAM[tid].push_back(new_entry);
		return;
	}
	...
	if(_replacementPolicy == RP_NLRU) {
		...
		itr = _cvuCAM[tid].begin();
		while(itr == itr_replace && itr != _cvuCAM[tid].end()) {
			itr++;
		} 
		itr_replace = itr;
		DPRINTF(CVU, "Entry with PC: 0x%x being ejected from CVU CAM", itr_replace->pc);
